...
%     in the next steps?
    ...
    % Structure to store the 'M' and 'l'
    scaling = cell(1, n_layer);
    ...
    for layer=(n_layer-1):-1:1
        for scale=1:n_scale(1,layer)
            ...
            if not(isempty(S{layer}.hmm{scale}.children))
                ...
                cond_up{layer}.M{scale} = sum(cond_up{layer}.Pvalue{scale} .* tmp_prodBeta ...
                    ./ (hidStates{layer}.ofHiddenStates{scale} ...
                    .^ (n_children-1)),...
                    3);
                ...
                %---------------------------------------------------------%
                cond_up{layer}.l{scale} = log(cond_up{layer}.M{scale})...
                    + tmp_sumL;
